...
- `Esc` or `b` - Back to tmux-ui management session (when inside tmux after switching)
- `x` - Detach from current session (exits tmux if already inside tmux, otherwise detaches all clients from selected session)
